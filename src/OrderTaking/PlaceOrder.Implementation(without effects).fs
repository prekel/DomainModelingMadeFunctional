...
type PlaceOrderWithoutEffects = UnvalidatedOrder -> PlaceOrderEvent list
...
module String50 =
    let create fieldName =
        String50.create fieldName >> failOnError

    let createOption fieldName =
        String50.createOption fieldName >> failOnError

module EmailAddress =
    let create fieldName =
        EmailAddress.create fieldName >> failOnError
...
module OrderLineId =
    let create fieldName =
        OrderLineId.create fieldName >> failOnError

module WidgetCode =
    let create fieldName =
        WidgetCode.create fieldName >> failOnError

module GizmoCode =
    let create fieldName =
        GizmoCode.create fieldName >> failOnError

module ProductCode =
    let create fieldName =
        ProductCode.create fieldName >> failOnError

module UnitQuantity =
    let create fieldName =
        UnitQuantity.create fieldName >> failOnError

module KilogramQuantity =
    let create fieldName =
        KilogramQuantity.create fieldName >> failOnError

module OrderQuantity =
    let create fieldName productCode =
        OrderQuantity.create fieldName productCode
        >> failOnError
...
type ValidateOrder =
    CheckProductCodeExists -> CheckAddressExists -> UnvalidatedOrder -> ValidatedOrder // input
...
type PriceOrder =
    GetProductPrice -> ValidatedOrder -> PricedOrder // input
...
type AcknowledgeOrder =
    CreateOrderAcknowledgmentLetter -> SendOrderAcknowledgment -> PricedOrder -> OrderAcknowledgmentSent option // output
...
let toAddress (checkAddressExists: CheckAddressExists) unvalidatedAddress =
    ...
    let city =
        checkedAddress.City |> String50.create "City"

    let zipCode =
        checkedAddress.ZipCode |> ZipCode.create "ZipCode"
...
let toPricedOrderLine (getProductPrice: GetProductPrice) (validatedOrderLine: ValidatedOrderLine) =
    let qty =
        validatedOrderLine.Quantity |> OrderQuantity.value

    let price =
        validatedOrderLine.ProductCode |> getProductPrice
